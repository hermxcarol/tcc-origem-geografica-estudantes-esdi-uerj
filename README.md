# Dados do TCC – Origem geográfica dos estudantes da ESDI/UERJ

Este repositório reúne as planilhas utilizadas na coleta, organização e análise dos dados do Trabalho de Conclusão de Curso apresentado ao curso de Design da Universidade do Estado do Rio de Janeiro (UERJ) entitulado "Trajetos: humanidades digitais e visualização de dados sobre o impacto do deslocamento na experiência universitária de alunos da ESDI/UERJ" de autoria de Carolina Guimarães sob orientação da Prof. Barbara Castro.

## Conteúdo
- Planilhas organizadas por ano de ingresso
- Planilha com relação bairro x ano
- Informações de bairro e cidade de origem dos estudantes

## Metodologia
Os dados foram coletados a partir de documentos institucionais e organizados manualmente em planilhas ao longo do processo de pesquisa.

## Observações
- Os dados não permitem identificação individual dos estudantes.
- As informações aqui disponibilizadas destinam-se exclusivamente a fins acadêmicos.
